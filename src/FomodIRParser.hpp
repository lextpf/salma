 ...
 *
 * 1. `parse` - entry point; extracts required files, steps, and
 *    conditional patterns from the document root.
 * 2. `compile_condition` - recursively converts `<dependencies>` /
 *    `<dependencyType>` nodes into a `FomodCondition` tree.
 * 3. `parse_file_entry` - converts a single `<file>` or `<folder>`
 *    element, applying prefix and destination resolution.
...
public:
     ...
     *                        contains the mod files). May be empty. The
     *                        parser normalizes joined source paths via
     *                        `mo2core::normalize_path`, so callers do not
     *                        need to pre-normalize this prefix.
