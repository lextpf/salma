...
/**
 * @fn std::string trim_copy(const std::string& s)
 * @brief Trim edge whitespace from metadata values.
 * @author Alex (<https://github.com/lextpf>)
 *
 * @param s Input bytes; whitespace uses the current C locale.
 * @return A copy with interior bytes unchanged.
 */
...
struct HandleGuard
{
    HANDLE h = nullptr;
    /**
     * @fn HandleGuard::HandleGuard()
     * @brief Initialize an empty Win32 handle owner.
     * @author Alex (<https://github.com/lextpf>)
     */
    HandleGuard() = default;
    /**
     * @fn HandleGuard::~HandleGuard()
     * @brief Close the owned handle when it is non-null.
     * @author Alex (<https://github.com/lextpf>)
     */
    ~HandleGuard()
    {
    ...
    /**
     * @fn HANDLE HandleGuard::release()
     * @brief Transfer the handle without closing it.
     * @author Alex (<https://github.com/lextpf>)
     *
     * @return The owned handle, or null when empty; the caller must close it.
     */
    HANDLE release()
    {
        auto tmp = h;
