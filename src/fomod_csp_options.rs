...
fn generate_raw_options(
    ...
    // externally-dynamic plugins are unknowable during standalone inference: keep them selectable
    // and do not force them Required.
    ...
    // evidence descending, index ascending on a tie. the tiebreak makes the order total, which is
    // what keeps the enumeration deterministic.
    ...
    match group.r#type {
        ...
        FomodGroupType::SelectAtLeastOne | FomodGroupType::SelectAny => {
            ...
            if n <= 10 && !force_heuristic_select_any {
                ...
                // pairs of non-Required usable plugins: all 16 or fewer candidates, else the top 8.
    ...
    // enforce intra-group dependencyType consistency using flags set by the option itself: drop
    // options where a selected plugin becomes NotUsable under the local flag overlay (unless it is
    // externally-dynamic).
    options.retain(|opt| {
...
// byte-exact signature of what an option produces: its atoms plus the flags it writes.
// both sorts exist because the source containers are unordered; without them the signature would
// vary between runs.
...
// which of two options with the same output signature to keep: higher evidence_score, then higher
// unique_support, then higher useful_dests, then lower extra_dests.
...
// move the candidate at rank into narrowed, if the rank is in range, not already chosen, and the
...
// if the filter would empty the list, every option is restored, so this stage can never reduce a
// non-empty list to nothing.
fn reduce_options(
    gref: &GroupRef,
    raw: &[GroupOption],
    pre: &Precompute<'_>,
...
// invalid group indices return shared empty options. domain pruning follows the nonempty fallback
// and can still produce an empty set.
pub fn get_options_for_group<'c>(
    ...
    // only compute on a cache miss (the reduction has stats side effects that must not be
    // double-counted on a hit), so this uses the Vacant-entry form rather than always constructing
    // the value.
    if let std::collections::hash_map::Entry::Vacant(slot) = cache.entry(key) {
        ...
        // emit the branching and group stats once per cache miss for this group, not once per
        // group: a group has one cache entry per (flag signature, effective cap, exact mode) tuple,
        // so it can log several times per solve. `logged_group_options` is written but never read,
        // so it suppresses nothing. the lookup is guarded because a caller may leave the counter
        // vector unsized.
...
mod tests {
    ...
    fn raw_options_heuristic_path_for_large_group() {
        // 11 usable plugins with positive evidence -> heuristic path (n > 10). the greedy "all
        // usable-with-evidence" option must be present, plus per-plugin singletons; and no invalid
        // options.
    ...
    fn raw_options_force_heuristic_gate_on_medium_no_evidence_select_any() {
        // pins the second trigger of the exhaustive-vs-heuristic gate, force_heuristic_select_any,
        // which no other option test exercises as true. the gate is `n <= 10 &&
        // !force_heuristic_select_any`, where force_heuristic_select_any is cap > 0 && SelectAny &&
        // !has_required && positive_evidence == 0 && n >= 8 so a medium (8..=10) no-evidence
        // SelectAny group takes the greedy and neighborhood heuristic even though n <= 10 would
        // otherwise enumerate the full powerset.
        ...
        // n = 8, cap > 0, no evidence, no required -> gate fires -> heuristic. heuristic emits the
        // empty greedy, 8 singletons, and the required-only (empty) option: 10 raw options, none
        // multi-select (no positive evidence -> no greedy multi-select and no pair candidates). the
        ...
        // n = 8 with a Required plugin -> gate's `!has_required` fails -> powerset. the required
        // bit 0 must be set, so 2^7 = 128 valid masks.
    ...
    fn reduce_options_select_any_cap_diversity_then_fill() {
        // construct > cap distinct-signature candidates and cap to 3. diversity sampling keeps rank
        // 0, then the first new selected-count at each rank, then fills by rank. assert exactly
        // `cap` kept and the cap counter. 4 plugins each producing a distinct useful dest, distinct
        // evidence so the ordering is total. SelectAny -> powerset (2^4 = 16 masks). with cap = 3
        // we must trim to 3.
        let installer = single_group(
    ...
    fn get_options_propagation_narrowing_drops_pruned_plugin() {
        // SelectExactlyOne over 2 plugins; propagation prunes plugin 1. options selecting plugin 1
        // must be dropped.
        let installer = single_group(
            FomodGroupType::SelectExactlyOne,
            vec![plugin("P0"), plugin("P1")],
